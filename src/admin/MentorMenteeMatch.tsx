...
const MentorMenteeMatch = () => {
  ...
  const [assignMenteeToMentorModal, setAssignMenteeToMentorModal] =
    useState<Mentor | null>(null);
  const [mentee, setMentee] = useState<MenteeFormNotes>();
  const [mentors, setMentors] = useState<Mentor[]>();
  const [notWaitingForEndpoints, setNotWaitingForEndpoints] =
    useState<boolean>();
  ...
  const getMentorsSortedByMatches = (mentors: Mentor[]) => {
    const mentorsWithMatches = mentors.map((mentor) => {
      ...
      if (mentee?.age) {
        ...
        for (const mentorAgePreference of mentor.agePreference) {
          const matchesElementary =
            mentorAgePreference === "Elementary" && menteeAge < 12;
          const matchesMiddleSchool =
            mentorAgePreference === "Middle School" &&
            menteeAge >= 12 &&
            menteeAge < 14;
          const matchesHighSchool =
            mentorAgePreference === "High School" &&
            menteeAge >= 14 &&
            menteeAge < 18;
      ...
      if (mentee?.interests) {
        matches += mentor.interestsAndHobbies.filter((interest) =>
          mentee.interests.includes(interest)
        ).length;
      }
      //best describes
      if (mentee?.bestDescribes) {
        matches += mentor.bestDescribes.filter((interest) =>
          mentee.bestDescribes.includes(interest)
        ).length;
      }
    ...
    return mentorsWithMatches.sort(
      (a: MentorWithMatches, b: MentorWithMatches) => {
        ...
        if (a.menteeIds && a.menteeIds.length > 0 && !a.canHaveManyMentees) {
          ...
          b.menteeIds &&
          b.menteeIds.length > 0 &&
          !b.canHaveManyMentees
        ) {
          return -1;
        } else {
  ...
  const getMentors = async () => {
    const data: Mentor[] = await NetworkManager.makeRequest(
      Endpoints.GetAllMentors
    );
    setMentors(data);
  };
  ...
  const onAssignMenteeToMentor = () => {
    ...
    if (!mentors) return;
    setAssignMenteeToMentorModal(
      getMentorsSortedByMatches(mentors)[selectedMentor]
    );
  };
  ...
  const assignConfirmed: VoidFunction = async () => {
    ...
    let mentor = getMentorsSortedByMatches(mentors)[selectedMentor];
    let numChars = mentee?.bestDescribes?.length || 0;
    try {
      setNotWaitingForEndpoints(false);
      await NetworkManager.makeRequest(Endpoints.MatchMentee, {
        menteeId: menteeId,
  ...
  const getClassNameForTrait = (
    propertyType: "bestDescribes" | "interests",
    trait: string
  ) => {
    let className = "trait";
    if (
      mentee &&
      mentee[propertyType] &&
      mentee[propertyType].indexOf(trait) !== -1
    ) {
      className += " match";
    }
    return className;
  ...
  const renderMentorList = (mentors: MentorWithMatches[]) => {
    if (mentors == undefined || mentors.length == 0) {
      return <tr>There are no mentors to display</tr>;
    }
    ...
    return mentors.map((mentor, idx) => (
      ...
      >
        ...
        <td className="mentor-table-cell">
          <div>{mentor.canHaveManyMentees}</div>
        </td>
        <td className="mentor-table-cell">
          <div>{mentor.agePreference?.join(", ")}</div>
        </td>
        <td className="mentor-table-cell">
          <div className="describes-you">
            {mentor.bestDescribes?.map((elem, i) => (
              <div
                key={i}
                className={getClassNameForTrait("bestDescribes", elem)}
              >
                {elem}
              </div>
            ))}
        ...
        <td className="mentor-table-cell mentor-table-cell-right">
          <div className="interests-hobbies">
            {mentor.interestsAndHobbies?.map((elem, i) => (
              <div key={i} className={getClassNameForTrait("interests", elem)}>
                {elem}
              </div>
            ))}
          </div>
  ...
  return (
    <div className="mentor-mentee-match">
      ...
      <div className="mentee-info-box">
        ...
        {mentee.oldMentorsList ? (
          <p className="mentee-info-text">
            Past Mentors:
            {mentee.oldMentorsList?.map((oldMentor, index) => {
              if (index == 0) {
                return " " + oldMentor.name;
              } else {
                return ", " + oldMentor.name;
              }
            })}
      ...
      <table className="mentor-table" ref={refs[2]}>
        ...
        <thead>
          <tr>
            ...
            <th className="mentor-table-header">
              Able To Have More Than One Mentee
            </th>
        ...
        {/* Mentor List */}
        <tbody>{renderMentorList(sortedMentorList)}</tbody>
      </table>
    </div>
  );
};
